...
def normalise_single_patient(patient_bp_df, bp_metrics=['systole', 'diastole', 'mitteldruck'],
                                datetime_column='timeBd'):
    for index, row in patient_bp_df.iterrows():
        time_of_bd = row[datetime_column]
        prior_bp_df = patient_bp_df[
            patient_bp_df[datetime_column] < time_of_bd]
        ...
        for bp_metric in bp_metrics:
            ...
            if (np.isnan(prior_medians[bp_metric])) & (not np.isnan(row[bp_metric])):
                patient_bp_df.loc[(patient_bp_df[datetime_column] == time_of_bd), f'{bp_metric}_normalised'] = 1
            else:
                patient_bp_df.loc[(patient_bp_df[datetime_column] == time_of_bd), f'{bp_metric}_normalised'] = normalisation(
...
def parallel_normalise(bp_df, bp_metrics=['systole', 'diastole', 'mitteldruck'],
                       pid_column='pNr', datetime_column='timeBd'):
    # add columns for normalised values
    for bp_metric in bp_metrics:
        bp_df[f'{bp_metric}_normalised'] = np.nan

    loop = asyncio.get_event_loop()  # Have a new event loop
    looper = tqdm.gather(*[normalise_single_patient(bp_df[bp_df[pid_column] == patient_id], bp_metrics, datetime_column)
                           for patient_id in bp_df[pid_column].unique()])  # Run the loop
...
def normalise(bp_df, bp_metrics=['systole', 'diastole', 'mitteldruck'],
    ...
    for patient_id in tqdm(bp_df[pid_column].unique()):
        patient_bp_df = bp_df[bp_df[pid_column] == patient_id]
        for index, row in patient_bp_df.iterrows():
            time_of_bd = row[datetime_column]
            prior_bp_df = patient_bp_df[
                patient_bp_df[datetime_column] < time_of_bd]
            ...
            for bp_metric in bp_metrics:
                ...
                if (np.isnan(prior_medians[bp_metric])) & (not np.isnan(row[bp_metric])):
                    bp_df.loc[(bp_df[pid_column] == patient_id) & (
                            bp_df[datetime_column] == time_of_bd), f'{bp_metric}_normalised'] = 1
                else:
                    bp_df.loc[(bp_df[pid_column] == patient_id) & (bp_df[
                            datetime_column] == time_of_bd), f'{bp_metric}_normalised'] = normalisation(
                                                                                    row[bp_metric], prior_medians[bp_metric])
