...
from utils.testing_utils import test_predictor, youdens_index, bootstrapped_roc_auc, safe_roc_auc_score


def test_bp_metrics(df, aim_for_sp80 = False):
    """
    ...
    """
    id_columns = ['negative_timebin', 'pNr', 'label', 'associated_event_time', 'timebin_size', 'normalized']
    ...
    for metric in tqdm(feature_columns):
        ...
        for train_index, test_index in skf.split(df, df['label']):
            ...
            train_df[metric].replace([np.inf, -np.inf], np.nan, inplace=True)
            train_df.dropna(subset=[metric], inplace=True)

            test_df = df.iloc[test_index]
            test_df[metric].replace([np.inf, -np.inf], np.nan, inplace=True)
            ...
            fold_test_roc_auc = safe_roc_auc_score(test_df['label'], test_df[metric])
            fold_train_roc_auc = safe_roc_auc_score(train_df['label'], train_df[metric])
        ...
        temp_df = df[['label', metric]]
        temp_df.replace([np.inf, -np.inf], np.nan, inplace=True)
        temp_df.dropna(subset=[metric], inplace=True)
        overall_roc_auc = safe_roc_auc_score(temp_df['label'], temp_df[metric])
        if overall_roc_auc < 0.5:
            overall_roc_auc = 1 - overall_roc_auc
        metric_results_df['overall_roc_auc'] = overall_roc_auc

        bs_median_roc_auc, bs_lower_ci_roc_auc, bs_upper_ci_roc_auc = bootstrapped_roc_auc(temp_df['label'], temp_df[metric])
...
def test_all_timebins(data_dir:str, aim_for_sp80 = False):
    ...
    for dir in os.listdir(data_dir):
        ...
        for data_filename in data_filenames:
            ...
            timebin_results_df = test_bp_metrics(df, aim_for_sp80=aim_for_sp80)
            timebin_results_df['timebin_size'] = df.timebin_size.unique()[0]
...
if __name__ == '__main__':
    ...
    results_df.to_csv(os.path.join(output_dir, 'bp_metrics_results.csv'), index=False)
